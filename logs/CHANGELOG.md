...
**问题**：用户在模块中途表达想停止（如「今天先这样，下次再聊」「我要上班了」），AI 温柔回应后却输出了 `[[NEXT_MODULE]]`，服务端把它当作本模块自然结束，直接展示下一模块开场白（例如又问起「你是在哪里长大的？」），体验非常突兀，像是没听见用户要离开。

**根因**：提示词规定非收尾模块只能用 `[[NEXT_MODULE]]`，AI 把「用户想停」当成「本模块完成」，输出了模块切换标记。

**改动目标**：用户明确想暂停/下次再聊时，AI 既不输出 `[[NEXT_MODULE]]` 也不输出 `[[INTERVIEW_END]]`，而是用 1-2 句话温柔回应并明确告诉用户「进度已保存，下次登录可继续」。会话保持 `IN_PROGRESS`，下次登录显示「继续访谈」自动接回原模块原进度。

**涉及文件**：
| 文件 | 改动说明 |
|------|----------|
| `lib/prompts/interviewer.txt` | 模块结束标记小节新增「用户想中途停止时」规则：禁止输出任何控制标记，回复中需告知进度已保存、下次可继续 |

**为什么不改后端**：单一 session 机制 + `IN_PROGRESS` 状态已经保证「下次回来就能续上」；只要 AI 不误输出控制标记，服务端的常规路径就会原样保留模块和进度，不需要新增后端分支。

**回溯方法**：删除 `lib/prompts/interviewer.txt` 中新增的「用户想中途停止时（极其重要）」整段即可。
...
**改动目标**：每个模块开场白只问一个具体、口语化的小问题，展开和追问交给 LLM 自然引导。

**涉及文件**：
| 文件 | 改动说明 |
|------|----------|
| `lib/modules.ts` | 6 个话题模块的 `cannedOpener` 全部精简 |

**改动对照**：

| 模块 | 改前 | 改后 |
|------|------|------|
| 人生故事 | 从童年开始讲人生故事，列举5个维度 | 你是在哪里长大的？ |
| 人生十字路口 | 解释概念+请讲来龙去脉 | 有没有一个决定改变了你后来的路？ |
| 重要的人 | 列举家人/朋友/老师/伴侣等类别 | 有没有一个人你特别想聊聊？ |
| 当下的生活 | 住哪+日常+和想象比较3问连发 | 最近的生活状态是怎么样的？ |
| 价值观与信念 | 3个问题叠加 | 你最在乎的是什么？ |
| 对未来的希望 | 想象几年后+有没有特别想做的事 | 往后的日子，你最想要的是什么？ |

**回溯方法**：还原 `lib/modules.ts` 中各模块的 `cannedOpener` 原文。

---

### [9] README 更新 + 推送 GitHub

**改动**：更新 README，反映 v1.1.0 功能（草稿机制、单一访谈限制、stone 品牌）；补充访谈流程说明；删除过时内容。
...
| 文件 | 改动说明 |
|------|----------|
| `lib/export-md.ts` | 导出 Markdown 中访谈者标签 `小灵` → `stone` |
| `app/layout.tsx` | 浏览器标签页标题 `小灵 · AI 访谈` → `stone · AI 访谈` |
